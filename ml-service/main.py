...
Environment Variables:
  ...
  TOKENIZER_PATH — path ke file .pkl (default: tokenizer.pkl)
"""
...
def _load_model():
    ...
    if not model_file.exists():
        logger.warning(f"⚠️  Model file '{MODEL_PATH}' tidak ditemukan. Inference tidak tersedia.")
    ...
    if not tokenizer_file.exists():
        logger.warning(f"⚠️  Tokenizer file '{TOKENIZER_PATH}' tidak ditemukan.")
        logger.warning("   Jalankan: python export_tokenizer.py (di folder ai/)")
        model = None
        return

    try:
        # Register StandaloneTokenizer agar pickle bisa deserialize
        # tokenizer.pkl yang dibuat oleh export_tokenizer.py (tanpa TensorFlow)
        import sys
        import tokenizer_compat
        # Pickle menyimpan module path tempat class di-define.
        # Tergantung cara export_tokenizer.py dijalankan, module-nya bisa:
        #   - '__main__' (jika python export_tokenizer.py)
        #   - 'export_tokenizer' (jika import export_tokenizer)
        # Kita register StandaloneTokenizer di kedua namespace.
        for mod_name in ("__main__", "export_tokenizer"):
            if mod_name not in sys.modules:
                import types
                sys.modules[mod_name] = types.ModuleType(mod_name)
            sys.modules[mod_name].StandaloneTokenizer = tokenizer_compat.StandaloneTokenizer

        with open(tokenizer_file, "rb") as f:
            tokenizer = pickle.load(f)
        logger.info(f"✅ Tokenizer loaded (vocab: {len(tokenizer.word_index)} words)")
        model_loaded = True
    except Exception as e:
...
app = FastAPI(
    ...
    description="Siamese BiLSTM inference untuk job-CV matching",
    version="1.0.0",
    lifespan=lifespan,
)
...
class RankRequest(BaseModel):
    ...
    jobs: list[JobPayload] = Field(..., min_length=1, description="Daftar lowongan")
...
def text_to_sequence(text: str) -> np.ndarray:
    """Konversi teks ke padded sequence."""
...
def batch_predict(cv_text: str, jobs: list[JobPayload]) -> list[float]:
    ...
    cv_padded = pad_sequences(cv_seq, maxlen=MAX_LENGTH, padding="post", truncating="post")

    # Siapkan semua job sequences
    job_texts = [
        f"{j.description} {' '.join(j.skills)}" for j in jobs
    ]
    job_seqs = tokenizer.texts_to_sequences(job_texts)
    job_padded = pad_sequences(job_seqs, maxlen=MAX_LENGTH, padding="post", truncating="post")
...
def compute_skill_match(cv_text: str, job_skills: list[str]) -> tuple[list[str], list[str]]:
...
def fallback_tfidf_rank(cv_text: str, jobs: list[JobPayload], top_k: int) -> list[dict]:
    ...
    for idx, job in enumerate(jobs):
        ...
        skill_score = len(matched) / max(len(set(s.lower() for s in job.skills)), 1)
...
async def rank_jobs_endpoint(req: RankRequest):
    ...
    # ── Fallback TF-IDF ──
    fallback_results = fallback_tfidf_rank(req.cv_text, req.jobs, req.top_k)
    return RankResponse(
...
async def get_trends():
    ...
    return TrendResponse(
        period="2026-Q2",
        trends=[
            TrendItem(skill="python", current_demand=0.85, predicted_demand=0.90, growth_pct=5.88),
            TrendItem(skill="machine learning", current_demand=0.72, predicted_demand=0.82, growth_pct=13.89),
            TrendItem(skill="react", current_demand=0.78, predicted_demand=0.81, growth_pct=3.85),
            TrendItem(skill="docker", current_demand=0.65, predicted_demand=0.74, growth_pct=13.85),
            TrendItem(skill="kubernetes", current_demand=0.45, predicted_demand=0.58, growth_pct=28.89),
            TrendItem(skill="typescript", current_demand=0.62, predicted_demand=0.70, growth_pct=12.90),
            TrendItem(skill="golang", current_demand=0.38, predicted_demand=0.50, growth_pct=31.58),
            TrendItem(skill="aws", current_demand=0.70, predicted_demand=0.76, growth_pct=8.57),
            TrendItem(skill="postgresql", current_demand=0.55, predicted_demand=0.60, growth_pct=9.09),
            TrendItem(skill="fastapi", current_demand=0.35, predicted_demand=0.48, growth_pct=37.14),
        ],
    )
